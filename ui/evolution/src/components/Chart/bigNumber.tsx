...
const BigNumber: React.FC<BigNumberProps> = ({ title, value, icon }) => {
    return (
        <div className="flex flex-col justify-between w-1/4 hover:shadow-2xl p-9 bg-white transition duration-1000 ease-in-out rounded-2xl shadow-lg">
            ...
            <p className="text-2xl font-extrabold">{value} <span className='text-sm leading-8 text-gray-500'>{title}</span></p>
        </div>
    );
};
